...
## Run locally
Prerequisites: Node.js 20 (LTS) and a running backend (`dotnet run --project ../backend/src/Mailgo.AppHost/Mailgo.AppHost.csproj`).
...
## Environment
- `REACT_APP_API_BASE_URL` (required): API base URL, e.g. `http://localhost:8080/api` for local dev or `http://localhost:8080/api` when using Docker.
