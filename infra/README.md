...
Notes:
- API is exposed on host port `5000` (container port `8080`). Frontend build is pinned to `http://localhost:5000/api` via `REACT_APP_API_BASE_URL` in `docker-compose.yml`. If you change the port, update that arg and rebuild the `web` image.
